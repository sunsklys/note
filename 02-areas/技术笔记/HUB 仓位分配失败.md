...
HUB 仓位分配失败  
事故类型：单订单永久卡死（仓位分配持续失败，无限重试无法自愈） 影响范围：单台机器出杯完全停摆，直到人工清理 Redis 锁 涉及模块：internal/server/DetailSpace.go、internal/service/ros/robot_system.go根因类型：双重缺陷叠加（锁清理缺陷 + 算法容错缺陷） 验证状态：已通过独立复现（runtime）、Oracle 对抗审查、路径核查三路交叉验证  
## 一、TL;DR
